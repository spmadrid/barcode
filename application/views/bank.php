...
$(document).ready(function() {
	 var selectedID;
		...
		{
				$('a.upd').attr("href","addnamic/tblbank");
		...
		function deleteClicked(com, grid) {
				...
				if (confirm('Delete '+selectedLength+' item[s]?')) {
					...
					$.ajax({
						type: 'POST',
						url: 'deletenames',
						data:{del_lead:selectedIDs,table:'tblbank'},
		...
		function editData(com,grid)
		{
			...
			var selectedID = getTextContent($('.trSelected')[0].cells[1]);
				...
				$('a.upd').attr("href","edit?id="+selectedID+"&table=tblbank");
				$('#call_link').fancybox().trigger('click');
		...
		{display:'Bank Name',name:'name',width:'300',sortable:true,align: 'left'},
		],
		buttons : [
		...
		searchitems : [
		{display: 'Bank', name : 'bank_name', isdefault: true},
		],
	sortname: "id",
	sortorder: "asc",
	usepager: true,
