...
$(document).ready(function() {
	 var selectedID;
		...
		{
				$('a.upd').attr("href","addnamic/tblbranch");
		...
		function deleteClicked(com, grid) {
				...
				if (confirm('Delete '+selectedLength+' item[s]?')) {
					...
					$.ajax({
						type: 'POST',
						url: 'deletenames',
						data:{del_lead:selectedIDs,table:'tblbranch'},
		...
		function editData(com,grid)
		{
			...
			var selectedID = getTextContent($('.trSelected')[0].cells[1]);
				...
				$('a.upd').attr("href","edit?id="+selectedID+"&table=tblbranch");
				$('#call_link').fancybox().trigger('click');
		...
		{display:'Branch',name:'name',width:'300',sortable:true,align: 'left'},
		],
		buttons : [
		...
		searchitems : [
		{display: 'Branch', name : 'branch_name', isdefault: true},
		],
	sortname: "id",
	sortorder: "asc",
	usepager: true,
