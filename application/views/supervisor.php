...
$(document).ready(function() {
	 var selectedID;
		...
		{
				$('a.upd').attr("href","addnamic/tblsupervisor");
		...
		function deleteClicked(com, grid) {
				...
				if (confirm('Delete '+selectedLength+' item[s]?')) {
					...
					$.ajax({
						type: 'POST',
						url: 'deletenames',
						data:{del_lead:selectedIDs,table:'tblsupervisor'},
		...
		function editData(com,grid)
		{
			...
			var selectedID = getTextContent($('.trSelected')[0].cells[1]);
				...
				$('a.upd').attr("href","edit?id="+selectedID+"&table=tblsupervisor");
				$('#call_link').fancybox().trigger('click');
		...
		$("#current").flexigrid({
			url : 'suplist?table=tblsupervisor',
			dataType:'json',
			colModel : [
		...
		{display:'Supervisor Name',name:'name',width:'300',sortable:true,align: 'left'},
		],
		buttons : [
